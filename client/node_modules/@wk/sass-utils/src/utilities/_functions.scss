////
/// @group wk-sass-utils.utilities.functions
...
@function wk-utils-dependencies($dependencies...) {
    ...
    @each $dependency in $dependencies {
        ...
        $exists-function: '#{$type}-exists';

        // sass version safety check
        @if function-exists('get-function') {
            $exists-function: get-function($exists-function);
        }

        @if not call($exists-function, $dependency) {
...
@function wk-component-dependencies($dependencies...) {
    ...
    @each $dependency in $dependencies {
        @if index($wk-imported-modules, $dependency) == null {
            $missing-dependencies: append($missing-dependencies, $dependency, comma);
        }
    }
...
///
@function wk-background-gradient($direction: right, $gradient-color: #fff) {
    $transparent-color: rgba($gradient-color, 0);
    @return (linear-gradient(to $direction, $gradient-color, $transparent-color) 0% 0%);
}

/// Returns the half gutter measurement of the super grid
///
@function wk-global-half-gutter() {
    @return $wk-global-gutters / 2;
}
